...
async function run() {
  ...
  const replacements = {
    '<<Nama Peserta>>': 'Budi Test IMGTAG'
  };
  const images = {
    '<<FOTOA>>': 'https://iili.io/CgLO5Mb.png',
    '<<FOTOB>>': 'https://iili.io/CgLO5Mb.png'
  };
  
  try {
      const res = await generatePdfFromTemplate(docId, folderId, replacements, "Test_IMGTAG_Output", images);
