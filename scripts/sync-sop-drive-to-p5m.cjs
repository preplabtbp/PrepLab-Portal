...
const SOP_ROOT_FOLDER_ID = '10fkhqkCKKqNduHxTPzkgDcB5T8jmDQYm';
...
async function main() {
  console.log('=== SYNCING SOP DOCUMENTS FROM GOOGLE DRIVE TO P5M MATERI ===\n');
  const client = await pool.connect();

  const rootItems = await getFolderFiles(SOP_ROOT_FOLDER_ID);
  ...
  for (const folder of subFolders) {
    ...
    for (const file of files) {
      if (file.mimeType === 'application/pdf' || file.name.endsWith('.pdf')) {
        ...
        const judulMateri = parseSOPTitle(file.name);
        const fileUrl = `https://drive.google.com/uc?export=view&id=${file.id}`;
        const notionId = `drive_sop_${file.id}`;
  ...
  console.log('================================================================');
  console.log(`🎉 SYNC SELESAI: ${processedResults.length} Dokumen SOP Drive Berhasil Dimigrasikan!`);
  console.log('================================================================');
