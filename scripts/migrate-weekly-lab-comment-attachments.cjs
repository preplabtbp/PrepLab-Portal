...
async function main() {
  ...
  console.log('=== Migrating Weekly Laboratorium (Post #542) Attachments to Google Drive ===');
  console.log('Target Google Drive Folder ID:', folderId);

  const dbId = '12cd00c5-c809-80a6-b4a9-e6fb1cc98cdc';
      ...
      if (uploadedAttachments.length > 0) {
        ...
        const updateRes = await client.query(`
          ...
          SET 
            file_url = $1,
            file_name = $2
          WHERE post_id = 542 AND topic_title = $3 AND (content = $4 OR created_at = $5)
        `, [
          attachmentsJson,
