...
async function migrateQuizData() {
  ...
  try {
    ...
    console.log(`Found ${devQuestions.rows.length} questions in Dev.`);
    
    if (devQuestions.rows.length > 0) {
      // Clear Prod quiz questions just in case
      await prodPool.query('DELETE FROM quiz_questions');
      
      console.log("Inserting into Prod...");
      for (const q of devQuestions.rows) {
        await prodPool.query(
          ...
          [q.id, q.category, q.text, JSON.stringify(q.options), q.correct_answer_index]
        );
      }
      // Update sequence
    ...
    console.log("Fetching app_settings QUIZ_CONFIG from Dev...");
