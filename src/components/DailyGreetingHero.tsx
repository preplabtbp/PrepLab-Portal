          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between pt-2.5 border-t border-black/10 dark:border-white/10 text-[11px] gap-2">
              <div className="flex items-center gap-1.5 opacity-90">
                ...
                {(() => {
                  const isAi = activeQuote.authorNik === '00000000000' || activeQuote.authorNik === 'AI' || String(activeQuote.authorName || '').toLowerCase().includes('generated by ai');
                  const author = isAi ? 'Generated by AI' : (activeQuote.authorName || 'Personil PrepLab');
                  return (
                    <div className="flex items-center gap-1.5">
                      <strong className={`font-bold flex items-center gap-1 ${isAi ? 'text-purple-400' : 'text-teal-400'}`}>
                        {isAi && <Sparkles className="w-3 h-3 text-amber-300 inline" />}
                        <span>{author}</span>
                      </strong>
                      {!isAi && activeQuote.authorRole && (
                        <span className="text-slate-400 hidden sm:inline">({activeQuote.authorRole})</span>
                      )}
                    </div>
                  );
                })()}
