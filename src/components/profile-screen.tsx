...
export function ProfileScreen({ 
  ...
  const calculateCuti = () => {
    ...
    for (const dateStr of sortedDates) {
      const d = parseStringDate(dateStr);
      if (d && d > today && sched[dateStr] === 'TRV') {
        futureTrv = d;
        break;
      }
    }
                      ...
                      <div className="bg-teal-50 p-3.5 rounded-xl border border-teal-100">
                        ...
                        {cutiInfo.actualCuti ? (
                           <div>
                              ...
                              {cutiInfo.hasTrv && (
                                 <p className="text-[10px] text-teal-600/80 mt-2 flex items-center gap-1">
                                    <Info className="w-3 h-3 shrink-0" /> Telah dijadwalkan oleh admin (Fixed TRV)
                                 </p>
                              )}
                           </div>
                        ) : (
                           <p className="text-sm text-teal-700">Belum ada estimasi jadwal cuti (Riwayat TRV tidak ditemukan)</p>
                        )}
                      </div>
                    </>
                  ) : (
