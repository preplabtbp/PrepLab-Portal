...
export function isAiQuote(q?: { authorNik?: string; authorName?: string } | null): boolean {
  ...
  const name = String(q.authorName || '').trim().toLowerCase();
  return nik === '00000000000' || nik === 'AI' || nik === 'ai' || name === 'generated by ai' || name.includes('generated by ai');
}
          ...
          {activeTab === 'details' && activeDetailQuote && (
            <div className="space-y-6 animate-in fade-in duration-150">
              ...
              >
                ...
                >
                  {(() => {
                    const isAi = isAiQuote(activeDetailQuote);
                    const displayName = isAi ? 'Generated by AI' : (activeDetailQuote.authorName || 'Personil PrepLab');
                    const displayRole = isAi ? 'AI Assistant' : (activeDetailQuote.authorRole || 'Staff');

                    return (
                      <div className="flex items-center gap-3">
                        <div 
                          className="w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-sm text-white shadow-xs shrink-0"
                          style={{ backgroundColor: isAi ? '#8B5CF6' : 'var(--primary, #0D9488)' }}
                        >
                          {isAi ? <Sparkles className="w-5 h-5 text-amber-300" /> : (displayName || 'U').charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-bold text-sm leading-tight flex items-center gap-1.5">
                            <span>{displayName}</span>
                            {isAi && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-purple-500/20 text-purple-300 font-mono border border-purple-500/30">
                                AI
                              </span>
                            )}
                          </p>
                          <p className="opacity-70 text-[11px]">
                            {isAi ? 'AI Assistant' : `${displayRole} • ${activeDetailQuote.authorSection || 'PrepLab'}`}
                            {!isAi && activeDetailQuote.authorNik && activeDetailQuote.authorNik !== '00000000000' && (
                  ...
                  {filteredQuotes.map(q => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {(() => {
                          const isAi = isAiQuote(q);
                          const displayName = isAi ? 'Generated by AI' : (q.authorName || 'Personil PrepLab');
                          const displayRole = isAi ? 'AI Assistant' : (q.authorRole || 'Staff');
                            ...
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                <div 
                                  ...
                                  style={{ backgroundColor: isAi ? '#8B5CF6' : 'var(--primary)' }}
                                >
                                  {isAi ? <Sparkles className="w-3.5 h-3.5 text-amber-300" /> : (displayName || 'U').charAt(0).toUpperCase()}
                                </div>
                                <div className="min-w-0">
                                  <p className="font-bold text-xs truncate flex items-center gap-1">
                                    <span>{displayName}</span>
                                    {isAi && (
                                      <span className="text-[8px] px-1 py-0.2 rounded-full bg-purple-500/20 text-purple-300 font-mono">
                                        AI
                                      </span>
                                    )}
                                  </p>
                                  <p className="text-[10px] opacity-60 truncate">{displayRole}</p>
                                </div>
                              </div>
