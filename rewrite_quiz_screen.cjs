...
export function QuizScreen({ onBack, userSection, inspectorName, inspectorNik }: { onBack: () => void, userSection: string, inspectorName: string, inspectorNik: string }) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-4 mt-4 pb-24 flex flex-col h-[calc(100vh-80px)]">
      {/* Header Info */}
      <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={onBack} className="w-10 h-10 p-0 rounded-full shrink-0">
      ...
      <Card className="p-6 shadow-sm border-slate-200 flex-1 flex flex-col">
