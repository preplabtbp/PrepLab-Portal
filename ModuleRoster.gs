function doPost(e) {
  // Supaya CORS tidak diblokir saat error, kita harus selalu me-return JSON yang valid
  try {
    ...
    // Return error sebagai JSON supaya tidak kena "Failed to fetch" (CORS block)
...
function findEmployeeInSheet(sheetName, nikToFind) {
  ...
  // Optimasi: Hanya baca kolom C (NIK) untuk pencarian yang cepat
  ...
  var joinDateColIdx = isStaff ? 12 : 12; // Crew: M (12) "Tanggal Bergabung Terbaru", L (11) "Awal"
  
  // Baca baris spesifik karyawan
  ...
  if (golColIdx !== -1) {
    ...
    // Kalau kolom GOL tidak ketemu di header, default ke kolom F (index 5) untuk crew
    gol = String(row[5] || "");
  }
